...
class CustomCommands(commands.Cog):
    ...
    async def add(self, interaction, key: str, *, message: str):
        ...
        if key in commands:
            await interaction.response.send_message(f"Command `{key}` already exists. Use `addresponse` to add another response.", ephemeral=True)
            return
        await self.bot.database.add_command(key, message)
        await interaction.response.send_message(f"Command `{key}` added.", ephemeral=True)
    ...
    async def add_response(self, interaction, key: str, *, message: str):
        ...
        await interaction.response.send_message(f"Response added to command `{key}`.", ephemeral=True)
    ...
    async def remove(self, interaction, key: str):
        ...
        async def confirm_remove(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(f"Command `{key}` removed.", ephemeral=True)
        await interaction.response.send_message(f"Are you sure you want to remove command `{key}` and all responses linked to it?", view=ConfirmView(confirm_remove, interaction), ephemeral=True)
