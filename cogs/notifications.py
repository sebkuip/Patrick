...
class RoleButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.role in interaction.user.roles:
            ...
            await interaction.response.send_message(f"{interaction.user.display_name}, you are no longer subscribed to {self.role.name} notifications.", ephemeral=True)
        else:
            await interaction.user.add_roles(self.role)
            await interaction.response.send_message(f"{interaction.user.display_name},you are now subscribed to {self.role.name} notifications.", ephemeral=True)
...
class Notifications(commands.Cog):
    ...
    async def cog_load(self):
        ...
        for category in self.bot.config["notifications"]:
            embed.add_field(name=category["name"], value=category["description"], inline=True)
