...
def is_staff():
    def predicate(ctx):
        if discord.utils.get(ctx.author.roles, id=ctx.bot.config["roles"]["staff"]) is not None:
            return True
        else:
...
def is_admin():
    def predicate(ctx):
        if discord.utils.get(ctx.author.roles, id=ctx.bot.config["roles"]["admin"]) is not None:
            return True
        else:
